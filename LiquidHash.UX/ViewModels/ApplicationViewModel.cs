using LiquidHash.CoinApi;
using LiquidHash.CoinApi.Data;
using LiquidHash.Discovery;
using LiquidHash.Discovery.Data;
using LiquidHash.Engine;
using LiquidHash.Engine.Data;
using LiquidHash.Engine.Data.Configuration;
using LiquidHash.Engine.Installers;
using LiquidHash.ExchangeApi.Data;
using LiquidHash.MobileMiner.Data;
using LiquidHash.MultipoolApi.Data;
using LiquidHash.Remoting;
using LiquidHash.Remoting.Broadcast;
using LiquidHash.Services;
using LiquidHash.Stats.Data;
using LiquidHash.Utility.Timers;
using LiquidHash.Utility.Net;
using LiquidHash.Utility.OS;
using LiquidHash.Utility.Serialization;
using LiquidHash.UX.Data;
using LiquidHash.UX.Data.Configuration;
using LiquidHash.UX.Extensions;
using LiquidHash.Xgminer;
using LiquidHash.Xgminer.Api;
using LiquidHash.Xgminer.Api.Data;
using LiquidHash.Xgminer.Data;
using LiquidHash.Xgminer.Discovery;
...
using System.Text;
using System.Timers;
using Application = LiquidHash.UX.Data.Configuration.Application;
using Broadcaster = LiquidHash.Remoting.Broadcast.Broadcaster;
using ConfigurationEventArgs = LiquidHash.UX.Data.ConfigurationEventArgs;
using Device = LiquidHash.Xgminer.Data.Device;
using Listener = LiquidHash.Discovery.Listener;
using Path = LiquidHash.Remoting.Data.Transfer.Configuration.Path;
using Perks = LiquidHash.UX.Data.Configuration.Perks;
using System.Globalization;

namespace LiquidHash.UX.ViewModels
{
    public class ApplicationViewModel
    {
        #region Constants
        private const int BfgMinerNotificationId = 100;
        private const int LiquidHashNotificationId = 102;
            ...
            Context.BeginInvoke((Action)(() =>
            {
                ...
                if (deviceInformationList != null)
                {
                    ...
                    if ((poolInformationList != null) &&
                        ...
                        //https://github.com/nwoolls/LiquidHash/issues/64
        ...
        //https://github.com/nwoolls/LiquidHash/issues/152
        ...
        {
            return System.IO.Path.Combine(System.IO.Path.GetTempPath(), "LiquidHash.Processes.xml");
        }
        ...
        private bool ConfigFileHandledForMiner(MinerDescriptor miner)
        {
            ...
            if (File.Exists(confFileFilePath))
            {
                ...
                PromptResult dialogResult = MessageBoxShow(String.Format("A {0} file has been detected in your miner directory. This file interferes with the arguments supplied by LiquidHash. Can LiquidHash rename this file to {1}?",
        ...
        private void RestartMiningLocally()
        {
            ...
            //force LiquidHash to apply updated mining strategies
            RefreshCoinStats();
        ...
        #region Stats API
        //public void SubmitLiquidHashStatistics()
        //{
        //    string installedVersion = LiquidHashInstaller.GetInstalledMinerVersion();
        ...
        //        Stats.ApiContext.SubmitMinerStatistics("http://LiquidHashstats.azurewebsites.net/api/", machineStat);
        ...
        //    string coinName)
        //{
        //    miningStatistics.MinerName = "LiquidHash";
        ...
        private void BroadcastHashrate()
        {
            ...
            catch (SocketException ex)
            {
                ...
                ShowLiquidHashRemotingError(ex);
            }
        }
        ...
        private void PerformRequestedCommand(string clientAddress, string signature, Action action)
        {
            ...
            if (!expectedSignature.Equals(signature))
            {
                ...
                {
                    //code to update UI
                    PostNotification("LiquidHash Remoting signature verification failed", NotificationKind.Danger);
                }), null);
        ...
        private void UpgradeLiquidHashRequested(object sender, RemoteCommandEventArgs ea)
        {
            string installedVersion, availableVersion;
            bool updatesAvailable = LiquidHashHasUpdates(out availableVersion, out installedVersion);
            if (!updatesAvailable)
                return;
        ...
        private void SetupRemotingEventHandlers()
        {
            ...
            ApplicationProxy.Instance.UpgradeLiquidHashRequested -= UpgradeLiquidHashRequested;
            ApplicationProxy.Instance.UpgradeLiquidHashRequested += UpgradeLiquidHashRequested;
        ...
        private void ShowLiquidHashRemotingError(SystemException ex)
        {
            Context.BeginInvoke((Action)(() =>
            {
                //code to update UI
                const string message = "LiquidHash Remoting communication failed";
        ...
        private void InstallLiquidHashRemotely()
        {
            //call ToList() so we can get a copy - otherwise risk:
            //System.InvalidOperationException: Collection was modified; enumeration operation may not execute.
            List<Instance> instancesCopy = InstanceManager.Instances.Where(i => i != InstanceManager.ThisPCInstance).ToList();
            foreach (Instance instance in instancesCopy)
            {
                ...
                PerformRemoteCommand(instance, service =>
                {
                    service.UpgradeLiquidHash(GetSendingSignature(closedInstance));
                });
            }
        }
        ...
        private static bool LiquidHashHasUpdates(out string availableVersion, out string installedVersion)
        {
            availableVersion = String.Empty;
            installedVersion = String.Empty;
            ...
            try
            {
                availableVersion = LiquidHashInstaller.GetAvailableMinerVersion();
            }
            catch (WebException)
            {
            ...
            installedVersion = LiquidHashInstaller.GetInstalledMinerVersion();
        ...
        private void InstallLiquidHashLocally()
        {
            if (ProgressStarted != null)
                ProgressStarted(this, new ProgressEventArgs
                {
                    Text = "Downloading and installing LiquidHash from " + LiquidHashInstaller.GetMinerDownloadRoot()
                });
            try
            {
                LiquidHashInstaller.InstallMiner(AppDomain.CurrentDomain.BaseDirectory);
            }
            finally
            {
                if (ProgressCompleted != null) ProgressCompleted(this, new EventArgs());
        ...
        public void CheckForDisownedMiners()
        {
            ...
            {
                PromptResult messageBoxResult = MessageBoxShow("LiquidHash has detected running miners that it does not own. Would you like to kill them?",
        ...
        private void CheckForLiquidHashUpdates()
        {
            ...
            if (updatesAvailable)
                DisplayLiquidHashUpdateNotification(availableVersion, installedVersion);
        }

        private void DisplayLiquidHashUpdateNotification(string availableMinerVersion, string installedMinerVersion)
        {
            PostNotification(LiquidHashNotificationId.ToString(),
                String.Format("LiquidHash version {0} is available ({1} installed)",
                ...
                () =>
                {
                    ...
                    if (allRigs)
                        InstallLiquidHashRemotely();

                    //this will restart the app
                    InstallLiquidHashLocally();
                }, NotificationKind.Information, "http://releases.LiquidHashapp.com");
        }
        ...
        private bool ShouldUpdateAllRigs()
        {
            ...
            if (RemotingEnabled && (InstanceManager.Instances.Count > 1))
            {
                PromptResult dialogResult = MessageBoxShow("Would you like to apply this update to all of your online rigs?",
                    "LiquidHash Remoting", PromptButtons.YesNo, PromptIcon.Question);
        ...
        public void UpdateBackendMinerAvailability()
        {
            ...
            catch (WebException ex)
            {
                ...
                //System.Net.WebException: The remote name could not be resolved: 'www.LiquidHashapp.com'
